#!/usr/bin/env bash
# gcal.sh — Gcal API via Okoro proxy with token session caching.
#
# Usage:
#   ./gcal.sh --endpoint <path> --intent <text> [--method GET] [--scope read] [--payload <json>]
#
# Required env:
...
TOKEN_CACHE="${TMPDIR:-/tmp}/okoro_gcal_${_token_prefix}_${SCOPE}.tok"
